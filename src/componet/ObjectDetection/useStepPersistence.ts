...
} from '../../types/objectDetection/training.ts';
...
export const useStepPersistence = (
    ...
    try {
      ...
      if (response.status === 200) {
        const { current_step, step_status } = response.data as {
          ...
          step_status?: Record<string, { status?: unknown; [k: string]: unknown }>;
        };
  ...
  }, [projectState?.name, projectState?.projectId, projectState?.version, token, userData?.activeUser?.userName]);

  const updateStepStatus = useCallback(
    async (stepName: StepKey, status: StepStatusValue, data: Record<string, unknown> = {}): Promise<void> => {
